...
async def test_end_to_end_cuboid(cuboid_fixture: str, request: pytest.FixtureRequest):
    cuboid = request.getfixturevalue(cuboid_fixture)

    step_asset = TestingAsset.from_asset(generate_cuboid_cad(cuboid))
    rendered_image = await step_renderer(step_asset)
    geom_props = geometric_property_analysis(cuboid)

    # Assert analysis ran
    assert (await download_asset(step_asset)).exists()
    assert (await download_asset(rendered_image)).exists()
